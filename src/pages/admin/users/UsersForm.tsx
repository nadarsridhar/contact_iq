...
function UsersForm() {
  ...
  const [webrtcFlag, setWebrtcFlag] = useState(CALL_TYPES[2].value);
  const [loginMode, setLoginMode] = useState(LOGIN_MODE[0].value);
  ...
  async function onSubmit(payload: z.infer<typeof userMasterSchema>) {
    try {
      ...
      switch (Number(form.getValues("UserCategory"))) {
        case UserCategory.Agent:
          payload.MappingTemplateId = "";
          break;

        case UserCategory.SUPER_ADMIN:
          payload.MappingTemplateId = "";
          payload.UserAlternateMobileNumber = "";
          break;
  ...
  async function fetchPrivilegeTemplateIds(
    ...
    try {
      ...
      if (!form.getValues("PrivilegeTemplateId")) {
        const value = data.find((d) => d.includes("DEFAULT")) ?? data[0];
  ...
  useEffect(() => {
    if (modalData && Object.keys(modalData)?.length > 0) {
      ...
      form.reset({
        ...
        LoginMode: String(LoginMode) ?? LOGIN_MODE[0].value,
      ...
      setLoginMode(String(LoginMode) ?? LOGIN_MODE[0]?.value);
